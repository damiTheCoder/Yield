...
export default function AssetDetail() {
  ...
  const description = asset.summary?.trim()
    ? asset.summary
    : "A liquidity-funded token with verifiable reserve, transparent discovery mechanics, and guaranteed floor value via CoinTag hunts. Track live liquidity, drill into cycle performance, and switch to artifact view to showcase the current featured collectible.";

  const chartData = useMemo(() => {
    const base = Math.max(currentLiquidity, 1);
    const points = 40;
    const rawSeed = (asset.ticker || asset.id)
      .split("")
      .reduce((acc, char, index) => acc + char.charCodeAt(0) * (index + 1), 0);
    let previous = base * 0.95;

    const series = Array.from({ length: points }).map((_, idx) => {
      const t = idx + rawSeed * 0.1;
      const swing = Math.sin(t * 0.45) * 0.55;
      const surge = Math.cos(t * 0.23) * 0.4;
      const turbulence = Math.sin((t + rawSeed) * 1.3) * 0.35;
      const progress = idx / Math.max(points - 1, 1);
      const drift = 1 + progress * 0.5; // bias upward across the series
      const multiplier = drift + swing + surge + turbulence * (1 - progress * 0.3);
      const target = Math.max(0.01, base * multiplier);
      const value = Number(((previous * 0.35 + target * 0.65)).toFixed(2));
      previous = value;
      return { label: `T${idx + 1}`, value };
    });
          ...
          <div className="space-y-10">
              ...
              {viewMode === "chart" ? (
                <>
                  ...
                  <div className="space-y-3">
                    ...
                    <ChartContainer config={chartConfig} className="w-full h-56 md:h-64">
                      <RechartsLineChart data={chartData} margin={{ left: 0, right: 0, top: 16, bottom: 8 }}>
            ...
            <section className="space-y-6">
              <div className="space-y-3">
                ...
                <p className="text-sm md:text-base text-muted-foreground/80 max-w-2xl">{description}</p>
              </div>
            </section>
          </div>
